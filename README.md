...
| High-performance mode (CPU governor `performance` on both clusters, NVIDIA persistence mode, thermal throttling kept) | — | `egpu-performance on` — opt-in in `install.sh` with `EGPU_PERFORMANCE=1` |
